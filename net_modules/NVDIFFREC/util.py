...
def latlong_to_cubemap(latlong_map, res,device="cuda"):
    ...
    return cubemap
def latlong_to_backmap(latlong_map, res,rotate_y_degree=0,device="cuda"):
    backmap = torch.zeros(1, res[0], res[1], latlong_map.shape[-1], dtype=torch.float32, device=device)
    s=5
    gy, gx = torch.meshgrid(torch.linspace(-1.0 + 1.0 / res[0], 1.0 - 1.0 / res[0], res[0], device=device), 
                            torch.linspace(-1.0 + 1.0 / res[1], 1.0 - 1.0 / res[1], res[1], device=device),
                            # indexing='ij')
    ...
    v = safe_normalize(cube_to_dir(s, gx, gy))
    if rotate_y_degree!=0:
        mtx = rotate_y(rotate_y_degree/180*torch.pi,device=device)[None]
        v = ru.xfm_vectors(v.view(1, v.shape[0] * v.shape[1], v.shape[2]), mtx).view(*v.shape)
        
    tu = torch.atan2(v[..., 0:1], -v[..., 2:3]) / (2 * np.pi) + 0.5
    tv = torch.acos(torch.clamp(v[..., 1:2], min=-1, max=1)) / np.pi
    texcoord = torch.cat((tu, tv), dim=-1)

    backmap[...] = dr.texture(latlong_map[None, ...], texcoord[None, ...], filter_mode='linear')[0]
    return backmap

def cubemap_to_latlong(cubemap, res,rotate_y_degree=0):
    ...
    if rotate_y_degree!=0:
        mtx = rotate_y(rotate_y_degree/180*torch.pi,device=device)[None]
        reflvec = ru.xfm_vectors(reflvec.view(1, reflvec.shape[0] * reflvec.shape[1], reflvec.shape[2]), mtx).view(*reflvec.shape)
